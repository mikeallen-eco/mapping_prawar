...
my_data <- read.csv("prawar_data/2023_class_data/prawar_class_data_2023.csv") %>%
...
sum_data %>%
  ...
  geom_sf(data = state_bounds, aes(), color = "gray",
          size = .25) +
  geom_tile(aes(x = grid_lon, y = grid_lat, fill = trill_rate), 
                 size = 1) +
  scale_fill_viridis(option = "inferno") +
  theme_bw() + 
  theme(text = element_text(size = 14)) +
...
ggsave("my_output/Fig4_mean_trill_rate.png", 
       height = 5, width = 5, dpi = 400)
```
